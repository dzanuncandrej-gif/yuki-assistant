...
```
микрофон → распознавание (faster-whisper) → агент с инструментами (Ollama)
        → реальное действие в системе → проверка результата → речь (Silero/Piper/Edge)
```
...
- **Файловые инструменты не заперты в одной папке.** Абсолютный путь примет любой
  из них; `search_scope: "home"` (по умолчанию) ограничивает только *поиск*, не
  чтение и не запись по прямо названному пути.
...
- **Зависимости не зафиксированы по хэшу.** `requirements.txt` задаёт нижние
  границы версий, не точные сборки. Модель Silero загружается через
  `torch.package` / `load_pickle` — это стандартный, но не защищённый от
  вредоносного файла формат; скачивайте модели только по ссылкам из этого репозитория.
