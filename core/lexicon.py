...
def correct_word(word: str, cutoff: float = 0.85) -> str:
    """Одно слово: возвращает словарное написание, если оно достаточно похоже.

    Порог поднят с 0.8: ниже него обычные слова разговора цеплялись за похожие по
    звучанию команды — «дела» превращалось в «сделай», «устал» в «удали». 0.85 всё
    ещё ловит реальные оговорки распознавания («скрыншот» → «скриншот»), но больше
    не трогает слова, которые просто случайно похожи.
    """
    lowered = word.lower()
