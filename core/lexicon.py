...
def correct(text: str) -> str:
    ...
    Само слово-триггер («найди», «напиши») тоже не трогается — оно короткое и его
    нет в словаре команд, поэтому нечёткий поиск подменял его на первое похожее
    слово из словаря (имя установленной программы, голоса): «найди» превращалось
    в «юнити», если в системе стоит Unity. Граница отсчитывается от начала
    триггера, а не от конца — раньше слово успевало попасть в исправляемую часть.
    """
    raw = str(text or "")
    if not raw.strip():
        return raw
    ...
    head_end = tail.start() if tail else len(raw)
