...
def voice_page(store: SettingsStore, actions: Actions) -> QWidget:
    ...
    items = [
        (profile.key, f"{profile.title} · {profile.character}"
         + ("" if profile.installed else "  (модель не скачана)"))
        for profile in catalog
    ]

    card = Card("Голос ассистента", "Тембр, которым Юки говорит вслух.")
    row, box = select_row("Профиль голоса", "Silero держит все голоса в одной модели — переключение мгновенное.",
    ...
    engine_row, engine = select_row(
        "Движок синтеза", "auto выбирает лучший доступный: Silero → Piper → Edge → системный.",
        [("auto", "АВТОМАТИЧЕСКИ"), ("silero", "SILERO · офлайн"), ("piper", "PIPER · офлайн"),
         ("edge", "EDGE · сеть"), ("pyttsx3", "СИСТЕМНЫЙ")],
        str(store.get("tts", "engine", "auto")),
    )
    engine.currentIndexChanged.connect(lambda: store.set("tts", "engine", engine.currentData()))
    card.add(engine_row)

    gain_row, gain, _ = slider_row("Громкость речи", "Уровень голоса относительно системного.",
    ...
    card.add(_buttons(
        _action("ПОСЛУШАТЬ", lambda: actions.preview_voice(str(box.currentData()))),
        _action("ТЕСТОВАЯ ФРАЗА", lambda: actions.say_test("Проверка голоса. Я слышу тебя и готова помочь."), ghost=True),
    ))

    listening = Card("Слух", "Как Юки слушает микрофон.")
    wake_row, wake = toggle_row("Обращение по имени", "Реагировать только после слова «Юки».",
                                bool(store.get("wake", "enabled", False)))
    wake.toggled.connect(lambda value: store.set("wake", "enabled", value))
    listening.add(wake_row)

    neural_row, neural = toggle_row("Нейронный детектор речи", "Silero VAD вместо порога громкости — меньше ложных срабатываний.",
    ...
    silence_row, silence, _ = slider_row("Пауза до конца фразы, мс", "Сколько тишины считать окончанием реплики.",
...
def language_page(store: SettingsStore, actions: Actions) -> QWidget:
    card = Card("Язык интерфейса", "На каком языке говорит меню и подписи.")
    row, box = select_row("Язык", "Пока доступны русский и английский.",
                          [("ru", "РУССКИЙ"), ("en", "ENGLISH")], str(store.get("ui", "language", "ru")))
    box.currentIndexChanged.connect(lambda: store.set("ui", "language", box.currentData()))
    card.add(row)

    speech = Card("Язык распознавания", "Что Юки ожидает услышать в микрофоне.")
    stt_row, stt = select_row("Речь", "Автоматический режим медленнее, но понимает обе речи.",
                              [("ru", "РУССКИЙ"), ("en", "ENGLISH"), ("auto", "АВТОМАТИЧЕСКИ")],
                              str(store.get("stt", "language", "ru")))
    stt.currentIndexChanged.connect(lambda: store.set("stt", "language", stt.currentData()))
    speech.add(stt_row)

    model_row, model = select_row("Модель распознавания", "Крупнее модель — точнее слова, но выше задержка.",
                                  [("tiny", "TINY · самая быстрая"), ("base", "BASE"), ("small", "SMALL · баланс"),
                                   ("medium", "MEDIUM"), ("large-v3-turbo", "LARGE V3 TURBO · точная")],
    ...
    lexicon_row, lexicon = toggle_row("Словарь исправлений", "Подгоняет расслышанные слова под имена программ и контактов.",
                                      bool(store.get("stt", "lexicon", True)))
    lexicon.toggled.connect(lambda value: store.set("stt", "lexicon", value))
    speech.add(lexicon_row)

    note = QLabel("Смена модели распознавания применяется при следующем запуске Юки.")
...
def ai_page(store: SettingsStore, actions: Actions) -> QWidget:
    card = Card("Модель", "Мозг ассистента: локальная Ollama.")
    model_row, model = input_row("Основная модель", "Например qwen3:8b — используется для сложных ответов.",
    ...
    fast_row, fast = input_row(
        ...
        str(store.get("brain", "fast_model", "")), "не задана",
    )
    fast.editingFinished.connect(lambda: store.set("brain", "fast_model", fast.text().strip()))
    card.add(fast_row)

    url_row, url = input_row("Адрес Ollama", "Где крутится сервер моделей.",
    ...
    behaviour = Card("Поведение", "Насколько свободно Юки рассуждает.")
    temp_row, temp, _ = slider_row("Творческая свобода", "Ниже — строже и предсказуемее ответы.",
    ...
    history_row, history, _ = slider_row("Память диалога, реплик", "Сколько прошлых реплик держать в контексте.",
    ...
    think_row, think = toggle_row("Показывать размышления",
                                  "Модель проговаривает ход мысли в журнале. Ответ становится в разы медленнее — включать только для отладки.",
                                  bool(store.get("brain", "think", False)))
    think.toggled.connect(lambda value: store.set("brain", "think", value))
    behaviour.add(think_row)

    proactive_row, proactive = toggle_row("Инициатива в звонке", "Сам предлагает помощь, увидев ошибку на экране.",
    ...
    behaviour.add(_buttons(
        _action("ОЧИСТИТЬ КОНТЕКСТ", actions.reset_context, ghost=True),
    ))

    senses = Card("Зрение и жесты", "Камера включается вместе с режимом видеосвязи.")
    camera_row, camera = toggle_row("Камера", "Узнаёт лицо и держит взгляд персонажа.",
    ...
    gestures_row, gestures = toggle_row("Управление жестами", "Пальцы управляют музыкой, громкостью и панелью.",
...
def character_page(store: SettingsStore, actions: Actions) -> QWidget:
    main = Card("Экранный компаньон", "Живой персонаж на рабочем столе: мимика, взгляд, голос.")
    enabled_row, enabled = toggle_row("Показывать персонажа", "Панель с персонажем поверх окон.",
    ...
    name_row, name = input_row("Имя", "Как обращаться к персонажу.",
    ...
    voice_row, voice = select_row("Голос персонажа", "Женские живые голоса с аниме-интонацией.",
                                  voice_items, current_voice)
    main.add(voice_row)

    look = Card("Внешний вид", "Размер, план и прозрачность фигуры на экране.")
    framing_row, framing = select_row(
        "План", "Насколько близко камера держит персонажа.",
        [("full", "В ПОЛНЫЙ РОСТ"), ("bust", "ПО ПОЯС"), ("close", "КРУПНО · ЛИЦО")],
        str(store.get("companion", "framing", "full")),
    )
    look.add(framing_row)

    quality_row, quality = select_row(
        "Качество картинки", "Ниже — меньше нагрузка на видеокарту.",
        [("high", "ВЫСОКОЕ"), ("medium", "СРЕДНЕЕ"), ("low", "ЛЁГКОЕ")],
        str(store.get("companion", "quality", "high")),
    )
    look.add(quality_row)

    scale_row, scale, _ = slider_row("Размер", "Насколько крупно персонаж стоит на столе.",
                                     float(store.get("companion", "scale", 1.0)) * 100, 60, 150)
    look.add(scale_row)

    opacity_row, opacity, _ = slider_row("Непрозрачность", "Полупрозрачная фигура меньше отвлекает.",
                                         float(store.get("companion", "opacity", 1.0)) * 100, 30, 100)
    look.add(opacity_row)

    top_row, on_top = toggle_row("Поверх всех окон", "Персонаж не прячется за другими программами.",
                                 bool(store.get("companion", "always_on_top", True)))
    look.add(top_row)

    through_row, click_through = toggle_row("Не перехватывать мышь", "Клики проходят сквозь панель к окнам под ней.",
                                            bool(store.get("companion", "click_through", False)))
    look.add(through_row)

    life = Card("Живость", "Из чего складывается ощущение живого существа.")
    idle_row, idle = toggle_row("Дыхание и микродвижения", "Покачивание корпуса, моргание, качание волос.",
                                bool(store.get("companion", "idle_motion", True)))
    life.add(idle_row)

    follow_row, follow = toggle_row("Следить за курсором", "Взгляд и голова поворачиваются за мышью.",
                                    bool(store.get("companion", "follow_cursor", True)))
    life.add(follow_row)

    lip_row, lip = toggle_row("Синхронизация рта", "Рот открывается по громкости собственной речи.",
                              bool(store.get("companion", "lip_sync", True)))
    life.add(lip_row)

    subs_row, subs = toggle_row("Реплики облаком", "Показывать сказанное над персонажем.",
    ...
    quiet_row, quiet, _ = slider_row(
        "Молчание до реплики, мин", "Сколько тишины должно пройти, прежде чем она заговорит.",
...
def account_page(store: SettingsStore, actions: Actions) -> QWidget:
    card = Card("Оператор", "Как Юки обращается к вам.")
    name_row, name = input_row("Имя", "Используется в приветствиях и напоминаниях.",
                               str(store.get("account", "name", "Оператор")), "Оператор")
    name.editingFinished.connect(lambda: store.set("account", "name", name.text().strip()))
    card.add(name_row)

    handle_row, handle = input_row("Ник", "Необязательно — пригодится для будущей синхронизации.",
                                   str(store.get("account", "handle", "")), "@nickname")
    handle.editingFinished.connect(lambda: store.set("account", "handle", handle.text().strip()))
    card.add(handle_row)

    city_row, city = input_row("Город", "Для погоды, если геолокация по IP ошибается.",
                               str(store.get("web", "default_city", "")), "Москва")
    city.editingFinished.connect(lambda: store.set("web", "default_city", city.text().strip()))
    card.add(city_row)

    scope_row, scope = select_row("Где искать файлы", "Область поиска по диску.",
                                  [("home", "ПАПКИ ПОЛЬЗОВАТЕЛЯ"), ("all", "ВСЕ ДИСКИ")],
    ...
    system = Card("Запуск", "Как Юки ведёт себя при входе в Windows.")
    ...
    status = Card("Состояние системы", "Что сейчас запущено.")
    report = QLabel(actions.diagnostics() or "Диагностика недоступна.")
    report.setObjectName("rowHint")
    report.setWordWrap(True)
    status.add(report)
    status.add(_buttons(_action("ОБНОВИТЬ", lambda: report.setText(actions.diagnostics()), ghost=True)))
